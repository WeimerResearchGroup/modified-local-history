...
class HistoryTreeProvider {
    ...
    getSettingsItem() {
        ...
        switch (this.contentKind) {
            case 1 /* EHistoryTreeContentKind.All */:
                return new HistoryItem(this, 'Search: all', 0 /* EHistoryTreeItem.None */, null, this.currentHistoryPath);
                break;
            case 0 /* EHistoryTreeContentKind.Current */:
                return new HistoryItem(this, 'Search: current', 0 /* EHistoryTreeItem.None */, null, this.currentHistoryFile);
                break;
            case 2 /* EHistoryTreeContentKind.Search */:
                return new HistoryItem(this, `Search: ${this.searchPattern}`, 0 /* EHistoryTreeItem.None */, null, this.searchPattern);
                break;
        }
    }
    getTreeItem(element) {
    ...
    getChildren(element) {
        return new Promise(resolve => {
            ...
            if (!element) { // root
                ...
                if (element.kind === 1 /* EHistoryTreeItem.Group */) {
                    this.historyFiles[element.label].forEach((file) => {
                        items.push(new HistoryItem(this, this.format(file), 2 /* EHistoryTreeItem.File */, vscode.Uri.file(file.file), element.label, true));
                    });
    ...
    loadHistoryFile(fileName, settings) {
        return new Promise((resolve, reject) => {
            ...
            this.controller.findGlobalHistory(pattern, this.contentKind === 0 /* EHistoryTreeContentKind.Current */, settings, this.noLimit)
                .then(findFiles => {
                // Current file
                if (this.contentKind === 0 /* EHistoryTreeContentKind.Current */) {
    ...
    loadHistoryGroups(historyFiles) {
        ...
        if (keys && keys.length > 0)
            keys.forEach((key) => {
                const item = new HistoryItem(this, key, 1 /* EHistoryTreeItem.Group */);
                this.tree[key] = { grp: item };
                items.push(item);
            });
        else
            items.push(new HistoryItem(this, 'No history', 0 /* EHistoryTreeItem.None */));
        return items;
    }
    ...
    showSide(element) {
        if (element.kind === 2 /* EHistoryTreeItem.File */)
    ...
    delete(element) {
        if (element.kind === 2 /* EHistoryTreeItem.File */)
            this.controller.deleteFile(element.file.fsPath)
                .then(() => this.refresh());
        else if (element.kind === 1 /* EHistoryTreeItem.Group */) {
    ...
    forCurrentFile() {
        this.contentKind = 0 /* EHistoryTreeContentKind.Current */;
        this.refresh();
    }
    forAll() {
        this.contentKind = 1 /* EHistoryTreeContentKind.All */;
        this.refresh();
    }
    forSpecificFile() {
        vscode.window.showInputBox({ prompt: 'Specify what to search:', value: '**/*myFile*.*', valueSelection: [4, 10] })
            ...
            if (value) {
                ...
                this.contentKind = 2 /* EHistoryTreeContentKind.Search */;
                this.refresh();
            }
        });
    }
...
class HistoryItem extends vscode.TreeItem {
    constructor(provider, label = '', kind, file, grp, showIcon) {
        super(label, kind === 1 /* EHistoryTreeItem.Group */ ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None);
        this.kind = kind;
        this.file = file;
        this.grp = this.kind !== 0 /* EHistoryTreeItem.None */ ? grp : undefined;
        switch (this.kind) {
            case 2 /* EHistoryTreeItem.File */:
        ...
        if (provider.contentKind === 0 /* EHistoryTreeContentKind.Current */) {
            this.command = this.kind === 2 /* EHistoryTreeItem.File */ ? {
